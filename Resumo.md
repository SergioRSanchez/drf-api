...
# Validações customizadas<h2>
Vimos que se passarmos um atributo vazio para o *PATCH* ele irá retornar um erro dizendo que não pode ser em branco, caso tenha passado esse atributo para mudança. Porém, alguns casos pedem uma validação mais personalizada, por exemplo, telefone só pode ter *números e alguns caracteres especiais*, ou então o horário do agendamento *não pode ser no passado*. Para isso criamos funções no *serializer.py*, com validações para nossos atributos, exemplo **def validate_data_horario**, e esse método recebe dois valores, o *self (instância do próprio serializer)* e o *value (que será automaticamente passado como data_horario, nesse caso)*. Aí dentro da função, nós fazemos as validações, nesse caso do data_horario, falamos que se o *value* passado pelo usuário for menor que o *timezone.now* (presente) ele vai lançar uma exceção dizendo *ValidationError* e uma mensagem de nossa preferência, e caso esteja tudo bem, retorna o próprio *value*. Dessa maneira criamos uma validação personalizada de um campo específico, que será chamada no processo de validação do nosso serializer.

Repare que utilizamos *timezone.now* acima, pois no nosso modelo a *data_horario* está usando fuso horário, repare que quando detalhamos um objeto, no horário consta um *Z* no final, indicando que está usando o fuso horário *UTC*. Portanto não poderíamos usar o *timezone* do Python, pois ele não trabalha com fuso horário.

Além das validações de campos como a feita acima, temos as validações **Object Level**, que dizem respeito a um serializer como um todo, não apenas a um campo dele. E para fazer isso nós sobrescrevemos o método *validate* que já existe no *serializer base*, e mais uma vez, quando começamos a digitar já é sugerido a implementação, que além do *self* é passado também o **attrs** (atributos, ou nesse caso, o *request data* que foi passado para o nosso serializer quando instancio ele). Nesse caso iremos verificar que, se o *email_usuario* passado terminar em *.br*, e o *telefone_usuario* conter o sinal **+**, obrigatoriamente deve ser seguido de **55** (código que representa o Brasil).